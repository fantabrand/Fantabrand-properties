...
export default function App({ Component, pageProps }) {
  ...
  // 🔥 LOADER WITH 30 MINUTE RESET
  useEffect(() => {
    if (!isHomePage) return;

    const lastSeen = localStorage.getItem("loaderLastSeen");
    const now = Date.now();

    const THIRTY_MINUTES = 30 * 60 * 1000;

    if (!lastSeen || now - parseInt(lastSeen) > THIRTY_MINUTES) {
      setLoading(true);
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          setLoading(false);
          localStorage.setItem("loaderLastSeen", now.toString());
        }, 600);
      }, 2200);
    }
  }, [isHomePage]);
    ...
    >
      ...
      {!loading && (
        <>
          ...
          <div style={{ flex: 1, paddingTop: !isAdminRoute ? "100px" : "0px" }}>
